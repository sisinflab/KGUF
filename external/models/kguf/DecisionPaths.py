...
class DecisionPaths:
    ...
    def save_mapped_features(self, features_map):
        ...
        dataset_path = os.path.abspath(os.path.join('./data', self.dataset_name, 'kguf', name))
    ...
    def save_item_features(self):
        ...
        item_feature_name = 'item_features' + str(self.npr) + "_" + str(self.criterion) + str(self.
                                                                                              depth) + str(
            self.seed) + ".pk"
        item_features_path = os.path.abspath(os.path.join('./data', self.dataset_name, 'kguf', item_feature_name))
        ...
        self.item_features = torch_sparse.SparseTensor(row=torch.tensor(row_indices, dtype=torch.int64),
                                                       ...
                                                       value=torch.tensor([r for i in
                                                                           [torch.ones(len(v)) / len(v) for _, v in
                                                                            self.item_features.items()] for r in i],
                                                                          dtype=torch.float64))
